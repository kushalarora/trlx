...
class AccelerateMPRORTrainer(AcceleratePPOTrainer):
    ...
    def make_experience_v1(self, num_rollouts: int = 1024, iter_count: int = 0):  # noqa:
        """Make experiences
        ...
        while len(ppo_rl_elements) < num_rollouts:
            ...
            samples = []
            # Generate samples from the language model (similar to using HuggingFace `generate` method)
            for input_ids, attention_mask, rollin_suffix in zip(batch.input_ids, batch.attention_mask, batch.rollin_suffixes):
                augmented_input_ids = torch.cat((input_ids, rollin_suffix)).unsqueeze(0)
                augmented_attention_mask = torch.cat((attention_mask, torch.ones_like(rollin_suffix))).unsqueeze(0)
                sample = self.generate(augmented_input_ids, augmented_attention_mask)
                samples.append(sample)
            
            orig_padding_side = self.tokenizer.padding_side
            self.tokenizer.padding_side = "right"
            samples = self.tokenizer.pad([{'input_ids': x[0]} for x in samples])['input_ids']
            self.tokenizer.padding_side = orig_padding_side

            stats["time/rollout_generate"] = time() - rollout_generate_time

            prompt_tensors = batch.input_ids
            device = prompt_tensors.device

            samples = samples.to(device)

            if len(prompt_tensors) != len(samples):
                pass
