...
def train(  # noqa: C901
    ...
    if reward_fn:
        ...
        pipeline = get_pipeline(config.train.pipeline)(
            ...
            config=config.method, is_eval=False, total_steps=config.train.epochs * config.method.num_rollouts//config.train.batch_size,
        )
    ...
    eval_pipeline = get_pipeline(config.train.pipeline)(
        eval_prompts, max_prompt_length, trainer.tokenizer, add_special_tokens=config.model.model_arch_type == "seq2seq", config=config.method, is_eval=True, total_steps=config.train.epochs * config.method.num_rollouts//config.train.batch_size,
    )
